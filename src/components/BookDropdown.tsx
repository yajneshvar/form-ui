...
export function BookDropdownAndSelectedBooks(props: BookDropdownProps) {
  ...
  const onUpdateBookQuantity = useCallback(
  ...
  return (
    <>
      ...
      <SelectedBooks
        ...
        onUpdated={onUpdateBookQuantity}
      />
    </>
  );
}
...
function SelectedBooks(props: SelectedBooksProps) {
  ...
  const renderRemovableCell = (params: GridCellParams) => (
    <IconButton
      onClick={(event: any) => {
        onDeleteBook(params.id.toString());
      }}
    >
...
export function BookDropdown(props: BookDropdownProps) {
  ...
  const onBooksChange = useCallback(
    (selectedBooks: SelectedBookQuantity[]) => {
    ...
    if (response?.ok) {
      response.json().then((listOfBooks) => {
        ...
        const uniqueTypes = new Set(
          retrievedBooks.map((retrievedBook) => retrievedBook.type)
        );
  ...
  const onAddBook = useCallback(() => {
    ...
    if (book !== null && itemToupdate === undefined) {
      onBooksChange([
        ...books,
        { book, startCount: quantity, endCount: null, netCount: null },
      ]);
      ...
      <Grid item xs={12} md={6}>
        {selectedType ? (
          <Autocomplete
            ...
            renderInput={(params) => (
              <TextField {...params} label="Choose a Book" variant="outlined" />
            )}
