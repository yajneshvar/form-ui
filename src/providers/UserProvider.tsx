...
export default function UserProvider(props: any) {
  const [userState, setUserState] = useState<AuthenticatedUser>(
    cachedCredentials?.credential?.user || null
  );
...
function RedirectOrRoute(props: RouteProps & AuthenticatedUserProps) {
  ...
  const initialAuthState: AuthenticationState = user
    ? { status: "LOGGED_IN" }
    : { status: "LOGGED_OUT" };
  const [authenticationStatus, dispatchAuthStatus] = useReducer(
    (authState: AuthenticationState, action: AuthenticationStatus) => {
      ...
      render={() => {
        if (authenticationStatus.status === "LOGGED_OUT") {
          return (
            <Route
              {...rest}
              render={({ location: redirectLocation }) => (
                <Redirect
                  to={{
                    pathname: "/login",
